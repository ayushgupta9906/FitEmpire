...
export default function PartnerScanner() {
  ...
  // Dynamically import CameraView for native mobile devices only
  let CameraViewComponent: any = null;
  if (Platform.OS !== 'web') {
    try {
      const expoCamera = require('expo-camera');
      CameraViewComponent = expoCamera.CameraView;
    } catch (e) {
  ...
  useEffect(() => {
    if (Platform.OS !== 'web') {
      try {
        ...
        Camera.requestCameraPermissionsAsync().then(({ status }: any) => {
