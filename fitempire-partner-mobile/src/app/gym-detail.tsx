...
export default function GymDetailScreen() {
  ...
  const fetchGymData = async (gymId: string) => {
    setLoading(true);
    try {
      const res = await gymsApi.getGymDetails(gymId);
      if (res.data?.data) {
        const g = res.data.data;
        setGym({
          name: g.name,
          category: g.category || 'PREMIUM FITNESS CENTER',
          avgRating: g.avgRating ? Number(g.avgRating).toFixed(1) : '4.9',
          totalReviews: g.totalReviews || 128,
          description: g.description || FALLBACK_GYM.description,
          phone: g.phone || FALLBACK_GYM.phone,
          websiteUrl: g.websiteUrl || FALLBACK_GYM.websiteUrl,
          coverImageUrl: g.coverImageUrl || FALLBACK_GYM.coverImageUrl,
        });

        if (g.branches && g.branches.length > 0) {
          setBranches(g.branches);
          setSelectedBranch(g.branches[0]);
        }
      }
    } catch (e) {
      console.warn('Could not fetch gym details from API, using fallback:', e);
    } finally {
      setLoading(false);
    }
  };
