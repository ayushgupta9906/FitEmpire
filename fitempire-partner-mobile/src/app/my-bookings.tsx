...
interface BookingRecord {
  ...
  locality: string;
  dateTime: string;
  status: 'Attended' | 'Cancelled' | 'Scheduled' | 'Confirmed';
  avatarLogo: string;
}
...
export default function MyBookingsScreen() {
  ...
  const fetchBookings = async () => {
    setLoading(true);
    try {
      const res = await bookingsApi.getMyBookings(0, 20);
      const data = res.data?.data?.content || [];
      if (data.length > 0) {
        const firstActive = data.find((b: any) => b.status === 'CONFIRMED' || b.status === 'SCHEDULED');
        if (firstActive) {
          setUpcomingBooking(firstActive);
        }

        const mapped: BookingRecord[] = data.map((b: any) => ({
          id: b.id,
          category: b.bookingType === 'SPORTS' ? 'sports' : 'studio',
          title: b.bookingType === 'CLASS' ? 'Class Session' : 'Gym Workout',
          centerName: b.gymName || 'FitEmpire Partner Center',
          locality: b.branchName || 'Main Arena',
          dateTime: b.bookingDate ? `${b.bookingDate}, ${b.startTime ? b.startTime.substring(0, 5) : 'Anytime'}` : 'Today',
          status: b.status === 'CHECKED_IN' ? 'Attended' : b.status === 'CANCELLED' ? 'Cancelled' : 'Scheduled',
          avatarLogo: 'https://images.unsplash.com/photo-1534438327276-14e5300c3a48?w=100&auto=format&fit=crop',
        }));
  ...
  const handleCancelBooking = (bookingId: string) => {
    Alert.alert('Cancel Workout', 'Are you sure you want to cancel this booking?', [
      { text: 'No, Keep It', style: 'cancel' },
      {
        ...
        onPress: async () => {
          ...
          if (upcomingBooking?.id === bookingId) {
            setUpcomingBooking(null);
          }
          Alert.alert('Booking Cancelled', 'Your workout slot has been cancelled.');
        },
      },
    ]);
  };

  const filteredPastBookings = bookingsList.filter((b) => {
